...
type AppState =
  ...
  | { screen: "results"; userScoringId: string; items: MatchResultItem[]; tookMs: number; selectedTaxonomies: string[] }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetchMatch(id, 20)
      .then((result) =>
        setState({ screen: "results", userScoringId: id, items: result.items, tookMs: result.tookMs, selectedTaxonomies: result.selectedTaxonomies })
      )
  ...
  async function handleSubmit(answers: { taxonomy_value_key: string }[], geo: { lat: number; lon: number } | null) {
    ...
    try {
      ...
      setState({ screen: "results", userScoringId: scoring.id, items: result.items, tookMs: result.tookMs, selectedTaxonomies: result.selectedTaxonomies });
    } catch (e) {
  ...
  if (state.screen === "results") {
    return (
      <MatchResults
        ...
        selectedTaxonomies={state.selectedTaxonomies}
        onBack={handleBack}
      />
    );
  }
