...
const Home = ({ widget, apiUrl, missions, total, request, environment }) => {
  ...
  useEffect(() => {
    ...
    if (widget.location) {
      return setFilters((f) => ({ ...f, location: widget.location }));
    }
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (filters.page > 1) {
        query.from = (filters.page - 1) * query.size;
      }
    ...
    >
      <header className={`w-full space-y-4 md:space-y-8 ${widget?.style === "carousel" ? "max-w-[1056px]" : "max-w-[1200px]"}`}>
        <div className="flex flex-col gap-2 md:flex-row md:justify-between">
          <h1 className="font-bold text-[28px] leading-[36px] md:p-0">
            {widget?.type === "volontariat" ? "Trouver une mission de Service Civique" : widget?.type === "benevolat" ? "Trouver une mission de Bénévolat" : "Trouver une mission"}
          </h1>
