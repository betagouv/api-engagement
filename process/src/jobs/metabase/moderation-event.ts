  ...
  try {
    ...
    while (true) {
      ...
      await prisma.mission
        .findMany({ where: { old_id: { in: data.map((hit) => hit.missionId) } }, select: { old_id: true, id: true } })
      ...
      for (const doc of data) {
        ...
        if (!res) continue;
        if (stored[doc._id.toString()] && !isDateEqual(stored[doc._id.toString()].updated_at, res.updated_at)) dataToUpdate.push(res);
      ...
      if (dataToCreate.length) {
        const res = await prisma.moderationEvent.createManyAndReturn({ data: dataToCreate, skipDuplicates: true });
