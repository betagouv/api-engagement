...
const downloadXlsx = async (url: string) => {
  try {
    ...
    const jobReporting = XLSX.utils.sheet_to_json(jobReportingWorksheet, { header: 1 }) as (
      | string
      | number
    )[][];
...
const getData = async (email: Email) => {
  try {
    ...
    if (!data) {
      captureException(
        "[Linkedin Stats] Failed to download",
        `Failed to download link in email ${email._id}`
      );
      return;
    }
    ...
    if (dateRangeIndex === -1) {
      captureException(
        "[Linkedin Stats] No date range found",
        `No date range found in email ${email._id}`
      );
      return;
    }
    const dateRange = data.overview[dateRangeIndex + 1][1]
      .split(" - ")
...
const handler = async () => {
  ...
  try {
    ...
    for (const email of emails) {
      ...
      console.log(
        `[Linkedin Stats] Created ${res.created} stats with ${res.failed.data.length} failed`
      );
    ...
    if (result.failed.data.length) {
      captureException(
        "[Linkedin Stats] Failed to create stats",
        `Failed to create stats, ${JSON.stringify(result.failed.data, null, 2)}`
      );
    }
  } catch (error: any) {
  ...
  console.log(
    `[Linkedin Stats] Ended at ${new Date().toISOString()} in ${(Date.now() - start.getTime()) / 1000}s`
  );
};
