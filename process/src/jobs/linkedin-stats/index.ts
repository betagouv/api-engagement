...
const handler = async () => {
  ...
  try {
    const emails = await EmailModel.find({ status: "PENDING", "to.email": "linkedin@api-engagement-dev.fr" });
    if (!emails.length) {
    ...
    for (const email of emails) {
      ...
      const exists = await EmailModel.exists({ status: "PROCESSED", date_from: data.from, date_to: data.to });
      if (exists) {
