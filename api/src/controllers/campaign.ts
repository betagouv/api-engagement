    ...
    try {
      const body = zod
        .object({
          ...
          trackers: zod
            .array(
              zod.object({ key: zod.string(), value: zod.string(), _id: zod.string().optional() })
            )
            .optional(),
        })
        .passthrough()
      ...
      if (!body.data.trackers || !body.data.trackers.length) {
        ...
        body.data.trackers.forEach((tracker: { key: string; value: string }) =>
          searchParams.append(tracker.key, tracker.value)
        );
      ...
      if (error.code === 11000) {
        return res
          .status(409)
          .send({ ok: false, code: RESSOURCE_ALREADY_EXIST, error: "Campaign already exists" });
      }
      next(error);
    }
  }
    ...
    try {
      ...
      if (body.data.toPublisherId && body.data.toPublisherId !== campaign.toPublisherId) {
        ...
        if (!prevToPublisher) {
          return res
            .status(404)
            .send({ ok: false, code: NOT_FOUND, error: "Previous publisher not found" });
        }
        if (!newToPublisher) {
          return res
            .status(404)
            .send({ ok: false, code: NOT_FOUND, error: "New publisher not found" });
        }
    ...
    try {
      ...
      const where = {
        bool: {
          ...
          should: [],
          filter: [
            { term: { "sourceId.keyword": campaign._id.toString() } },
            { term: { "fromPublisherId.keyword": prevFromPublisher._id.toString() } },
          ],
        },
      } as EsQuery;
