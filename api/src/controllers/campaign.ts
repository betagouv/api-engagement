...
router.post("/", passport.authenticate("admin", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!fromPublisher) return res.status(404).send({ ok: false, code: NOT_FOUND, error: "Publisher not found" });

    const toPublisher = await PublisherModel.findById(body.data.toPublisherId);
    if (!toPublisher) return res.status(404).send({ ok: false, code: NOT_FOUND, error: "Publisher not found" });
    ...
    if (body.data.url) {
      if (body.data.url.indexOf("http") === -1) body.data.url = `https://${body.data.url}`;
      if (!isValidUrl(body.data.url)) return res.status(400).send({ ok: false, code: INVALID_BODY, error: "Invalid url" });
      else body.data.url = body.data.url;
    }
...
router.post("/:id/duplicate", passport.authenticate("admin", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!campaign) return res.status(404).send({ ok: false, code: NOT_FOUND, error: "Campaign not found" });
...
router.put("/:id", passport.authenticate("admin", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (body.data.name) campaign.name = req.body.name;
    if (body.data.type) campaign.type = req.body.type;
    if (body.data.trackers && body.data.trackers.length) campaign.trackers = body.data.trackers;
    else {
      ...
      if (body.data.url) body.data.url = `${body.data.url}${body.data.url.includes("?") ? "&" : "?"}${searchParams.toString()}`;
      else campaign.url = `${campaign.url}${campaign.url.includes("?") ? "&" : "?"}${searchParams.toString()}`;
    }
    if (body.data.url) {
      if (body.data.url.indexOf("http") === -1) body.data.url = `https://${body.data.url}`;
      if (!isValidUrl(body.data.url)) return res.status(400).send({ ok: false, code: INVALID_BODY, error: "Invalid url" });
      else campaign.url = body.data.url;
    }
    if (body.data.active !== undefined) campaign.active = body.data.active;
    ...
    if (body.data.toPublisherId && body.data.toPublisherId !== campaign.toPublisherId) {
      ...
      if (!prevToPublisher) return res.status(404).send({ ok: false, code: NOT_FOUND, error: "Previous publisher not found" });
      if (!newToPublisher) return res.status(404).send({ ok: false, code: NOT_FOUND, error: "New publisher not found" });
...
router.put("/:id/reassign", passport.authenticate("admin", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (campaign.fromPublisherId === body.data.fromPublisherId) {
      return res.status(400).send({ ok: false, code: INVALID_BODY, error: "Campaign is already assigned to this publisher" });
    }
    ...
    if (!prevFromPublisher) return res.status(404).send({ ok: false, code: NOT_FOUND, error: "Previous publisher not found" });
    if (!newFromPublisher) return res.status(404).send({ ok: false, code: NOT_FOUND, error: "New publisher not found" });
...
router.delete("/:id", passport.authenticate("admin", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const campaign = await CampaignModel.findById(params.data.id);
    if (!campaign) return res.status(200).send({ ok: true });
