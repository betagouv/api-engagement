...
router.post("/search", passport.authenticate("user", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!body.success) return res.status(400).send({ ok: false, code: INVALID_BODY, error: body.error.errors });

    if (req.user.role !== "admin" && (!body.data.publisherId || !req.user.publishers.includes(body.data.publisherId))) return res.status(403).send({ ok: false, code: FORBIDDEN });
...
router.get("/", passport.authenticate("user", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (queryError) return res.status(400).send({ ok: false, code: INVALID_PARAMS, error: queryError.details });

    if (req.user.role !== "admin" && !query.publisherId) return res.status(403).send({ ok: false, code: FORBIDDEN });

    const where = {} as { [key: string]: any };
    if (query.publisherId) {
      if (req.user.role !== "admin" && !req.user.publishers.includes(query.publisherId)) return res.status(403).send({ ok: false, code: FORBIDDEN });
      else where.publisherId = query.publisherId;
    } else if (req.user.role !== "admin") where.publisherId = { $in: req.user.publishers };
