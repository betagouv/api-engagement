...
import { PublisherModel, MissionModel } from "@shared/models";
...
router.get("/views", passport.authenticate("user", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (query.data.type === "volontariat") where.bool.filter.push({ term: { "toPublisherName.keyword": "Service Civique" } });
    else if (query.data.type === "benevolat") where.bool.filter.push({ bool: { must_not: { term: { "toPublisherName.keyword": "Service Civique" } } } });
        ...
        aggs: {
          clicks_per_period: {
            ...
            aggs: {
              ...
              benevolat: {
                filter: { bool: { must_not: { term: { "toPublisherName.keyword": "Service Civique" } } } },
              },
            },
          },
          applies_per_period: {
    ...
    if (response.statusCode !== 200) return next(response.body.error);
      ...
      },
      {
        $facet: {
          ...
          histogram: [
            {
              $group: {
                _id: { $dateToString: { format: interval === "day" ? "%Y-%m-%d" : "%Y-%m", date: "$createdAt" } },
                total: { $sum: 1 },
...
router.get("/active-missions", passport.authenticate("user", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (query.data.type === "volontariat") where.publisherName = "Service Civique";
    else if (query.data.type === "benevolat") where.publisherName = { $ne: "Service Civique" };
...
router.get("/publishers-views", passport.authenticate("user", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    else if (query.data.to) viewQuery.bool.filter.push({ range: { createdAt: { lte: query.data.to } } });
    ...
    if (query.data.type === "volontariat") viewQuery.bool.filter.push({ term: { "toPublisherName.keyword": "Service Civique" } });
    else if (query.data.type === "benevolat") viewQuery.bool.filter.push({ bool: { must_not: { term: { "toPublisherName.keyword": "Service Civique" } } } });
