  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const aggs = {
        period: {
          date_histogram: {
            field: "createdAt",
            calendar_interval:
              query.data.to && query.data.from
                ? (query.data.to.getTime() - query.data.from.getTime()) / (1000 * 60 * 60 * 24) > 62
                  ? "month"
                  : "day"
                : "month",
      ...
      const data = {
        ...
        histogram: response.body.aggregations.period.buckets.map(
...
router.get(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const createdFacet = await MissionModel.aggregate([
        ...
        },
        {
          $facet: {
            ...
            benevolat: [
              { $match: { publisherName: { $ne: "Service Civique" } } },
              { $count: "total" },
            ],
            histogram: [
              {
      ...
      const data = {
        activeVolontariatMissions: activeFacet[0].volontariat.length
          ? activeFacet[0].volontariat[0].total || 0
          : 0,
        activeBenevolatMissions: activeFacet[0].benevolat.length
          ? activeFacet[0].benevolat[0].total || 0
          : 0,
        ...
        totalVolontariatMissions: createdFacet[0].volontariat.length
          ? createdFacet[0].volontariat[0].total || 0
          : 0,
        totalBenevolatMissions: createdFacet[0].benevolat.length
          ? createdFacet[0].benevolat[0].total || 0
          : 0,
...
router.get(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const $facet =
        interval === "day"
          ? buildDaysFacets(query.data.from, query.data.to)
          : buildMonthFacets(query.data.from, query.data.to);
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const data = publishers.map((p) => ({
        ...
        clickFrom:
          response.body.aggregations.clickFrom.data.buckets.find(
            (b: { key: string; doc_count: number }) => b.key === p.name
          )?.doc_count || 0,
        clickTo:
          response.body.aggregations.clickTo.data.buckets.find(
            (b: { key: string; doc_count: number }) => b.key === p.name
          )?.doc_count || 0,
        applyFrom:
          response.body.aggregations.applyFrom.data.buckets.find(
            (b: { key: string; doc_count: number }) => b.key === p.name
          )?.doc_count || 0,
        applyTo:
          response.body.aggregations.applyTo.data.buckets.find(
            (b: { key: string; doc_count: number }) => b.key === p.name
          )?.doc_count || 0,
      }));
      ...
      const total = {
        ...
        broadcasters: publishers.filter((e) => {
          const isBroadcaster =
            e.role_annonceur_api || e.role_annonceur_campagne || e.role_annonceur_widget;
          if (!isBroadcaster) {
            return false;
          }
          if (query.data.type === "volontariat") {
