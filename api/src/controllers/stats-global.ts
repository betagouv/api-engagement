...
router.get("/announce-preview", async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (query.data.publisherId) {
      where.bool.filter.push({ term: { "toPublisherId.keyword": query.data.publisherId } });
    }
    if (query.data.from) {
      where.bool.must.push({ range: { createdAt: { gte: query.data.from } } });
    }
    if (query.data.to) {
      where.bool.must.push({ range: { createdAt: { lte: query.data.to } } });
    }
...
router.get(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (query.data.type) {
        where.bool.filter.push({ term: { type: query.data.type } });
      }
...
router.get("/evolution", async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (query.data.type) {
      where.bool.filter.push({ term: { type: query.data.type.toString() } });
    }
    ...
    const body = {
      ...
      aggs: {
        topPublishers: {
          terms: {
            field:
              query.data.flux === "to" ? "fromPublisherName.keyword" : "toPublisherName.keyword",
            size: 4,
          },
        },
        histogram: {
          ...
          aggs: {
            publishers: {
              terms: {
                field:
                  query.data.flux === "to"
                    ? "fromPublisherName.keyword"
                    : "toPublisherName.keyword",
                size: 80,
              },
            },
          },
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const data = response.body.aggregations.by_announcer.buckets.map((bucket: any) => ({
        ...
        rate: bucket.total_click.count.value
          ? bucket.total_apply.count.value / bucket.total_click.count.value
          : 0,
      }));
