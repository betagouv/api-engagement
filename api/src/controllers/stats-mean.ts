...
router.get("/", passport.authenticate("user", { session: false }), async (req: UserRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (query.data.source === "widget") where.bool.filter.push({ term: { "source.keyword": "widget" } });
    else if (query.data.source === "campaign") where.bool.filter.push({ term: { "source.keyword": "campaign" } });
    else where.bool.filter.push({ bool: { minimum_should_match: 1, should: [{ term: { "source.keyword": "publisher" } }, { term: { "source.keyword": "jstag" } }] } });
