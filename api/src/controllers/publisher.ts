  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (errorQuery) {
        return res
          .status(400)
          .send({ ok: false, code: INVALID_QUERY, message: errorQuery.details });
      }
      ...
      if (query.partnersOf) {
        if (user.role !== "admin" && !user.publishers.find((e: string) => e === query.partnersOf)) {
          ...
          where["publishers.publisher"] = query.partnersOf;
        }
      }
      if (query.moderatorOf) {
        if (
          user.role !== "admin" &&
          !user.publishers.find((e: string) => e === query.moderatorOf)
        ) {
          return res.status(403).send({ ok: false, code: FORBIDDEN, message: `Not allowed` });
        } else {
...
router.post(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const where = { deletedAt: null } as { [key: string]: any };
      if (body.data.role_promoteur) {
        where.role_promoteur = body.data.role_promoteur;
      }
      if (body.data.name) {
      ...
      if (body.data.partnersOf) {
        if (
          req.user.role === "admin" ||
          (req.user.role !== "admin" &&
            req.user.publishers.some((e: string) => e === body.data.partnersOf))
        ) {
      ...
      if (!where._id && !where["publishers.publisher"] && req.user.role !== "admin") {
        where._id = { $in: req.user.publishers };
      }
...
router.get(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        req.user.role !== "admin" &&
        !req.user.publishers.find(
          (e: string) => e === params.data.id || publisher.publishers.find((p) => p.publisher === e)
        )
      ) {
        return res.status(403).send({ ok: false, code: FORBIDDEN, message: `Not allowed` });
      }
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (body.data.annonceur && !body.data.missionType) {
        return res
          .status(400)
          .send({ ok: false, code: INVALID_BODY, message: "Mission type is required" });
      }
      if (body.data.diffuseur && !body.data.category) {
        return res
          .status(400)
          .send({ ok: false, code: INVALID_BODY, message: "Category is required" });
      }
      ...
      if (body.data.automated_report !== undefined) {
        publisher.automated_report = body.data.automated_report;
      }
      if (body.data.send_report_to) {
        publisher.send_report_to = body.data.send_report_to;
      }
