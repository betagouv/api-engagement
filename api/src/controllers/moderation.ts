...
router.post(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        req.user.role !== "admin" &&
        !req.user.publishers.includes(body.data.publisherId) &&
        !req.user.publishers.includes(body.data.moderatorId)
      ) {
...
router.post(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const data = {
        status: facets[0].status
          .filter((b: { _id: string }) => b._id)
          .map((b: { _id: string; count: number }) => ({ key: b._id, doc_count: b.count })),
        comments: facets[0].comments
          .filter((b: { _id: string }) => b._id)
          .map((b: { _id: string; count: number }) => ({ key: b._id, doc_count: b.count })),
...
router.post(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
            ...
            association: [
              {
                $group: {
                  ...
                  acceptedCount: {
                    $sum: {
                      $cond: [
                        { $eq: [`$moderation_${body.data.moderatorId}_status`, "ACCEPTED"] },
                        1,
                        0,
                      ],
                    },
                  ...
                  refusedCount: {
                    $sum: {
                      $cond: [
                        { $eq: [`$moderation_${body.data.moderatorId}_status`, "REFUSED"] },
                        1,
                        0,
                      ],
                    },
      ...
      response[0].association.forEach(
        (b: { _id: string; count: number; acceptedCount: number; refusedCount: number }) => {
      ...
      response[0].organization.forEach(
        (b: { _id: string; count: number; acceptedCount: number; refusedCount: number }) => {
