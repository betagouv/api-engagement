  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const where = {
        bool: {
          must: [],
          must_not: [{ term: { "missionId.keyword": "" } }, { term: { isBot: true } }],
          should: [],
          filter: [{ exists: { field: "missionId" } }],
        },
      } as EsQuery;

      where.bool.filter.push({
        term: { [`${query.data.flux}PublisherId.keyword`]: query.data.publisherId },
      });
        ...
        aggs: {
          ...
          missions: {
            terms: {
              ...
              order: ["print", "click", "apply", "account"].includes(query.data.sort || "")
                ? { [query.data.sort]: "desc" }
                : undefined,
            },
...
router.get(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (query.data.sort === "clickCount") {
        sort.bucket_sort.sort = [{ "click._count": "desc" }];
      } else if (query.data.sort === "applyCount") {
...
router.get(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const data = response.body.aggregations.domains.buckets.filter(
        (b: { key: string }) => b.key !== ""
      );
...
router.get(
  ...
  async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (query.data.from && query.data.to) {
        where.createdAt = { $gte: query.data.from, $lte: query.data.to };
      } else if (query.data.from) {
        where.createdAt = { $gte: query.data.from };
      } else if (query.data.to) {
        where.createdAt = { $lte: query.data.to };
      }
      ...
      const facets = await MissionModel.aggregate([
        { $match: where },
        {
          $facet: {
            ...
            refused: [
              { $match: { statusCode: "REFUSED" } },
              { $group: { _id: "$statusComment", count: { $sum: 1 } } },
              { $sort: { count: -1 } },
              { $limit: 4 },
            ],
          },
      ...
      const data = {
        ...
        comments: facets[0].refused.map((r: { _id: string; count: number }) => ({
          comment: r._id,
          rate: facets[0].status.find((s: { _id: string }) => s._id === "REFUSED")?.count
            ? r.count / facets[0].status.find((s: { _id: string }) => s._id === "REFUSED")?.count
            : 0,
        })),
        rate: 0,
      };
      if (data.accepted + data.refused) {
        data.rate = data.accepted / (data.accepted + data.refused);
      }
