    ...
    try {
      ...
      const body = {
        ...
        aggs: {
          ...
          publishers: {
            terms: {
              field: query.data.flux === "to" ? "fromPublisherId.keyword" : "toPublisherId.keyword",
              size: 1000,
            },
            aggs: {
              ...
              name: {
                top_hits: {
                  _source: {
                    includes: [query.data.flux === "to" ? "fromPublisherName" : "toPublisherName"],
                  },
                  size: 1,
                },
              },
      ...
      const data = {
        performance: {
          printCount:
            response.body.aggregations.type.buckets.find((b: { key: string }) => b.key === "print")
              ?.doc_count || 0,
          clickCount:
            response.body.aggregations.type.buckets.find((b: { key: string }) => b.key === "click")
              ?.doc_count || 0,
          applyCount:
            response.body.aggregations.type.buckets.find((b: { key: string }) => b.key === "apply")
              ?.doc_count || 0,
          accountCount:
            response.body.aggregations.type.buckets.find(
              (b: { key: string }) => b.key === "account"
            )?.doc_count || 0,
          rate:
            response.body.aggregations.type.buckets.find((b: { key: string }) => b.key === "rate")
              ?.doc_count || 0,
        },
        publishers: response.body.aggregations.publishers.buckets
          ...
          .map((b: { [key: string]: any }) => ({
            id: b.key,
            name: b.name.hits.hits[0]._source[
              query.data.flux === "to" ? "fromPublisherName" : "toPublisherName"
            ],
