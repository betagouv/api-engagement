    ...
    try {
      ...
      if (body.data.lat && body.data.lon) {
        const distance = getDistanceKm(
          body.data.distance && body.data.distance !== "Aucun" ? body.data.distance : "25km"
        );
        where.geoPoint = {
          $nearSphere: {
            $geometry: {
        ...
        whereAggs.geoPoint = {
          $geoWithin: {
            $centerSphere: [
              ...
              body.data.distance && body.data.distance !== "Aucun"
                ? getDistanceKm(body.data.distance)
                : getDistanceKm("50km") / EARTH_RADIUS,
            ],
          },
        };
      }
      ...
      const facets = await MissionModel.aggregate([
        { $match: whereAggs },
        {
          $facet: {
            ...
            leboncoinStatus: [
              { $group: { _id: "$leboncoinStatus", count: { $sum: 1 } } },
              { $sort: { count: -1 } },
            ],
          },
        },
      ...
      const aggs = {
        ...
        partners: facets[0].partners.map(
          (b: { _id: string; count: number; publisherName: string }) => ({
            _id: b._id,
            count: b.count,
