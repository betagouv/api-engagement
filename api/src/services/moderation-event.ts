...
const buildModerationEventPayload = (previous: Mission, update: Mission, user: User, moderatorId: string): ModerationEventCreateInput => {
  const data = {
    moderatorId,
...
export const moderationEventService = {
  ...
  buildModerationEventPayload,

  async logModeration(previous: Mission, update: Mission, user: User, moderatorId: string) {
    const payload = buildModerationEventPayload(previous, update, user, moderatorId);
    await this.createModerationEvent(payload);
  },
};
