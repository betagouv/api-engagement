...
// Snapshot statique des champs indexés à l'instant de la v6.
// Changement vs v5 : les facettes `domaine` / `secteur_activite` (taxonomies antérieures) sont
// remplacées par les taxonomies du quiz `domaine_engagement` / `activite`.
// Pour ajouter ou retirer des champs, créer un v7.ts — ne pas modifier ce fichier.
export const MISSION_TAXONOMY_FIELDS_V6: TaxonomyKey[] = ["domaine_engagement", "activite", "type_mission", "tranche_age", "dispositif"];
const taxonomyFields = MISSION_TAXONOMY_FIELDS_V6;
