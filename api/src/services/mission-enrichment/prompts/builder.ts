...
const boolLabel = (value: boolean | null): string =>
  value === true ? "Oui" : value === false ? "Non" : "Non précisé";

export const buildTaxonomyBlock = (taxonomy: TaxonomyForPrompt): string =>
  taxonomy
    .map(
      (dim) =>
        `### ${dim.key} — ${dim.label} (type: ${dim.type})\n` +
...
export const buildMissionBlock = (mission: MissionForPrompt): string => {
  ...
  if (mission.tasks.length) lines.push("", "**Tâches :**", mission.tasks.join("\n"));
  if (mission.audience.length) lines.push("", "**Public cible :**", mission.audience.join("\n"));
  if (mission.softSkills.length) lines.push("", "**Compétences souhaitées :**", mission.softSkills.join("\n"));
  if (mission.requirements.length) lines.push("", "**Prérequis :**", mission.requirements.join("\n"));
  ...
  if (mission.organizationName) {
    ...
    if (mission.organizationDescription) lines.push(`**Description :** ${mission.organizationDescription}`);
    if (mission.organizationActions.length) lines.push(`**Actions :** ${mission.organizationActions.join(", ")}`);
    if (mission.organizationBeneficiaries.length) lines.push(`**Bénéficiaires :** ${mission.organizationBeneficiaries.join(", ")}`);
    if (mission.organizationParentOrganizations.length) lines.push(`**Organisations parentes :** ${mission.organizationParentOrganizations.join(", ")}`);
    if (mission.organizationObject) lines.push(`**Objet social :** ${mission.organizationObject}`);
    if (mission.organizationSocialObject1) lines.push(`**Objet social 1 :** ${mission.organizationSocialObject1}`);
    if (mission.organizationSocialObject2) lines.push(`**Objet social 2 :** ${mission.organizationSocialObject2}`);
  }
