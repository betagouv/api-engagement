...
export const buildSystemPrompt = (taxonomy: TaxonomyForPrompt): string => {
  const dimensionsBlock = taxonomy
    .map(
      (dim) =>
...
export const buildUserMessage = (mission: MissionForPrompt): string => {
  ...
  lines.push(
    `**Ouverte aux mineurs :** ${mission.openToMinors === true ? "Oui" : mission.openToMinors === false ? "Non" : "Non précisé"}`,
  );
  lines.push(
    `**Accessible PMR :** ${mission.reducedMobilityAccessible === true ? "Oui" : mission.reducedMobilityAccessible === false ? "Non" : "Non précisé"}`,
  );
