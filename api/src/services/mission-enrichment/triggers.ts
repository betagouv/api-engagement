 ...
 * que si un champ réellement consommé par le prompt a changé. `deletedAt` et
 * `statusCode` forcent aussi le retraitement pour propager suppression,
 * restauration et retrait de l'index de recherche.
 */
export const changesRequireEnrichment = (changes: Record<string, unknown>): boolean =>
  Object.keys(changes).some((field) => field === "deletedAt" || field === "statusCode" || ENRICHMENT_TRIGGER_FIELD_SET.has(field));
