...
export const buildQuery = (
  rules: { field: string; operator: string; value: string; combinator: "and" | "or" }[]
) => {
  const q = { bool: { must: [], must_not: [], should: [], filter: [] } } as EsQuery;
  rules.forEach(
    (r: { field: string; operator: string; value: string; combinator: string }, i: number) => {
      if (
        !r.field ||
        !r.operator ||
        (!r.value && r.operator !== "exists" && r.operator !== "does_not_exist")
      ) {
        return;
      }
