...
const handler = async () => {
  try {
    ...
    await prismaClient.kpi.findMany({ select: { old_id: true } }).then((data) => data.forEach((d) => stored.push(d.old_id)));
    ...
    if (dataToCreate.length) {
      ...
      const res = await prismaClient.kpi.createMany({ data: dataToCreate, skipDuplicates: true });
