  ...
  async (req: PublisherRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (paramsError) {
        return res
          .status(400)
          .send({ ok: false, code: INVALID_PARAMS, message: paramsError.details });
      }
  ...
  async (req: PublisherRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (bodyError) {
        return res
          .status(400)
          .send({ ok: false, code: INVALID_PARAMS, message: bodyError.details });
      }
      ...
      if (body.logo) {
        association.logo = body.logo;
      }
      if (body.url) {
        association.url = body.url;
      }
      if (body.linkedin) {
        association.linkedin = body.linkedin;
      }
      if (body.facebook) {
        association.facebook = body.facebook;
      }
      if (body.twitter) {
        association.twitter = body.twitter;
      }

      if (body.description) {
        association.description = body.description;
      }
      if (body.donation) {
        association.donation = body.donation;
      }
      if (body.statut_juridique) {
        association.statut_juridique = body.statut_juridique;
      }
      if (body.domaines) {
        association.domaines = body.domaines;
      }
      if (body.publics_beneficiaires) {
        association.publics_beneficiaires = body.publics_beneficiaires;
      }

      if (body.tags) {
        if (Array.isArray(body.tags)) {
          association.tags = body.tags;
        } else {
          association.tags = [...new Set([...(association.tags || []), body.tags])];
        }
      }

      if (body.coordonnees?.courriel) {
        if (Array.isArray(body.coordonnees.courriel)) {
          association.coordonnees_courriel = body.coordonnees.courriel;
        } else {
          association.coordonnees_courriel = [
            ...new Set([...(association.coordonnees_courriel || []), body.coordonnees.courriel]),
          ];
        }
      }
      if (body.coordonnees?.telephone) {
        if (Array.isArray(body.coordonnees.telephone)) {
          association.coordonnees_telephone = body.coordonnees.telephone;
        } else {
          association.coordonnees_telephone = [
            ...new Set([...(association.coordonnees_telephone || []), body.coordonnees.telephone]),
          ];
        }
      }
      ...
      if (body.coordonnees?.adresse) {
        ...
        if (adresse.location) {
          association.coordonnees_adresse_location = adresse.location;
        }
        if (adresse.nom_complet) {
          association.coordonnees_adresse_nom_complet = adresse.nom_complet;
        }
        if (adresse.nom) {
          association.coordonnees_adresse_nom = adresse.nom;
        }
        if (adresse.code_postal) {
          association.coordonnees_adresse_code_postal = adresse.code_postal;
        }
        if (adresse.code_insee) {
          association.coordonnees_adresse_code_insee = adresse.code_insee;
        }
        if (adresse.rue) {
          association.coordonnees_adresse_rue = adresse.rue;
        }
        if (adresse.numero) {
          association.coordonnees_adresse_numero = adresse.numero;
        }
        if (adresse.commune) {
          association.coordonnees_adresse_commune = adresse.commune;
        }
        if (adresse.commune_ancienne) {
          association.coordonnees_adresse_commune_ancienne = adresse.commune_ancienne;
        }
        if (adresse.contexte) {
          association.coordonnees_adresse_contexte = adresse.contexte;
        }
        if (adresse.departement_numero) {
          association.coordonnees_adresse_departement_numero = adresse.departement_numero;
        }
        if (adresse.departement) {
          association.coordonnees_adresse_departement = adresse.departement;
        }
        if (adresse.region) {
          association.coordonnees_adresse_region = adresse.region;
        }
        if (adresse.type) {
          association.coordonnees_adresse_type = adresse.type;
        }
      }
    ...
    try {
      ...
      if (body.city) {
        where.query.bool.must.push({
          bool: {
            should: [
              { match: { coordonnees_adresse_commune: { query: body.city, boost: 7 } } },
              { match: { coordonnees_adresse_siege_commune: { query: body.city, boost: 3 } } },
            ],
          },
        });
      }
      ...
      if (body.minScore) {
        where.min_score = body.minScore;
      }
      if (body.size) {
        where.size = body.size;
      }

      const result = await esClient.search({ index: ASSOS_INDEX, body: where });
      const data = result.body.hits.hits.map(
        (e: { _id: string; _score: number; _source: Association }) =>
          toPublic({ _id: e._id, score: e._score, ...e._source })
      );
...
const toPublic = (data: Association) => {
  return {
    ...
    publics_beneficiaires: data.publics_beneficiaires?.length
      ? data.publics_beneficiaires
      : undefined,
