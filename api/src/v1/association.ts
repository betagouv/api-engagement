...
router.put("/:rnaId/etablissement/:id", passport.authenticate(["apikey", "api"], { session: false }), async (req: PublisherRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (bodyError) {
      return res.status(400).send({ ok: false, code: INVALID_PARAMS, message: bodyError.details });
    }
    ...
    if (body.coordonnees?.courriel) {
      if (Array.isArray(body.coordonnees.courriel)) {
        ...
        association.coordonnees_courriel = [...new Set([...(association.coordonnees_courriel || []), body.coordonnees.courriel])];
      }
    }
    if (body.coordonnees?.telephone) {
      if (Array.isArray(body.coordonnees.telephone)) {
        ...
        association.coordonnees_telephone = [...new Set([...(association.coordonnees_telephone || []), body.coordonnees.telephone])];
      }
    }
