  ...
  async (req: PublisherRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const hitsQuery = {
        bool: {
          filter: [
            { bool: { should: SHOULD_EXIST.map((e) => ({ exists: { field: e, boost: 2 } })) } },
          ],
        },
      } as { [key: string]: any };
      const facetsQuery = {
        ...
        if (body.filters.coordonnees_adresse_region.includes("N/A")) {
          filter.bool = {
            should: [
              ...
              {
                terms: {
                  "coordonnees_adresse_region.keyword":
                    body.filters.coordonnees_adresse_region.filter((e: string) => e !== "N/A"),
                },
              },
            ],
          };
        ...
        if (body.filters.coordonnees_adresse_departement.includes("N/A")) {
          filter.bool = {
            should: [
              ...
              {
                terms: {
                  "coordonnees_adresse_departement.keyword":
                    body.filters.coordonnees_adresse_departement.filter((e: string) => e !== "N/A"),
                },
              },
            ],
          };
        ...
        if (body.filters.activites_lib_theme1.includes("N/A")) {
          filter.bool = {
            should: [
              ...
              {
                terms: {
                  "activites_lib_theme1.keyword": body.filters.activites_lib_theme1.filter(
                    (e: string) => e !== "N/A"
                  ),
                },
              },
