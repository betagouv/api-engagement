...
Scripts utilitaires pour insérer des données de démonstration directement dans la base. Ils se lancent depuis `api/` avec `npx tsx`.
...
- **Prérequis**:
  ...
  - `npm install` déjà effectué (tsx, prisma).
- **Commandes**:
  ```bash
  # Aperçu des données générées
  cd api
  npx tsx scripts/fixtures/populate-stat-events-from-missions.ts --dry-run

  # Insertion réelle (par défaut 25 StatEvents par type et par publisher cible)
  npx tsx scripts/fixtures/populate-stat-events-from-missions.ts

  # Modifier le volume (ex: 40 événements par type)
  npx tsx scripts/fixtures/populate-stat-events-from-missions.ts --per-type 40

  # Cibler un publisher précis
  npx tsx scripts/fixtures/populate-stat-events-from-missions.ts --publisher-id 5f5931496c7ea514150a818f
  ```
- **Options**:
