...
const DEPT_OVERRIDES: Record<string, string[]> = {
  ...
  "2A": ["corse du sud", "corse-du-sud", "sis2a", "sis 2a"],
  "2B": ["haute-corse", "haute corse", "sis2b", "sis 2b"],
...
export function findPublisherForDept(publishers: { id: string; name: string; apikey: string | null }[], dept: string): { id: string; name: string; apikey: string | null } | null {
  ...
  return (
    publishers.find((p) => {
      ...
      if (normalized !== paddedDept && (nameLower.includes(` ${normalized} `) || nameLower.includes(` ${normalized}`) || nameLower.endsWith(normalized))) return true;
      return false;
    }) ?? null
  );
}
