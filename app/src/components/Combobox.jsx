import { ComboboxButton, ComboboxInput, ComboboxOption, ComboboxOptions, Combobox as HLCombobox, Transition } from "@headlessui/react";
...
const Combobox = ({
  ...
  const selected = filteredOptions.find((o) => o[by] === value) || null;
  ...
  return (
    <HLCombobox as="div" className="relative w-full" value={selected} onChange={(e) => (e ? onSelect(e) : null)} by={by}>
      <div className="relative w-full">
        ...
        />
        <ComboboxButton className="absolute inset-y-2 right-0 flex items-center pr-2" aria-label="Ouvrir les options">
          {({ open }) => <RiArrowDownSLine className={`text-sm ${open ? "rotate-180 transform" : ""}`} />}
        </ComboboxButton>
      </div>

      <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
        <ComboboxOptions className={`absolute top-10 z-10 max-h-80 w-full overflow-y-auto bg-white shadow-lg ${className || "w-full"}`}>
          {loading ? (
