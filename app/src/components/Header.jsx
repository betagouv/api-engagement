...
const Header = () => {
  ...
  return (
    <header className="flex w-full justify-center border-b border-b-[#dddddd] bg-white">
      <div className="w-full max-w-7xl flex items-center justify-between py-3">
        <Link className="flex items-center gap-4 p-4 hover:bg-gray-975" to={user ? "/" : "/login"}>
        ...
        <div className="flex items-center gap-3 text-sm text-blue-france">
          <div className="flex items-center gap-2 p-2 hover:bg-gray-975">
            <RiBookletLine />
          ...
          {!user ? (
            <Link to="/login" className="button flex cursor-pointer items-center border border-gray-900 hover:bg-gray-975">
...
const NotificationMenu = () => {
  ...
  return (
    <Menu>
      <MenuButton as="div" className="relative p-2 text-lg data-[focus]:bg-gray-975 hover:bg-gray-975">
        <RiDashboard3Line />
        {warnings.length > 0 && <div className="absolute right-1.5 top-2 h-[9px] w-[9px] rounded-full border border-white bg-[#ff5655]" />}
      </MenuButton>
      ...
      >
        <MenuItem>
          <div className="flex items-center justify-between p-6">
            ...
            <Link to="/warning" className="flex items-center text-blue-france">
              <span>Détails</span>
        ...
        {state && (
          <MenuItem>
            <Link to="/warning" className="flex items-center justify-between gap-6 p-6 hover:bg-gray-950 border-t border-gray-900">
        ...
        {warnings.length ? (
          <>
                ...
                <MenuItem key={w._id}>
                  <Link to="/warning" className="flex items-center justify-between gap-6 p-6 hover:bg-gray-950 border-t border-gray-900">
                    ...
                    <div className="flex flex-1 flex-col gap-2">
                      ...
                      <p className="m-0 text-xs text-gray-425">{new Date(w.createdAt).toLocaleDateString("fr-FR")}</p>
                    </div>
            ...
            {warnings.length > 2 && (
              <MenuItem>
                <Link to="/warning" className={`flex items-center justify-end gap-6 p-6 hover:bg-gray-950 border-t border-gray-900`}>
                  <div className="flex text-blue-france">
...
const AdminNotificationMenu = () => {
  ...
  return (
    <Menu>
      <MenuButton className="relative p-2 text-lg data-[focus]:bg-gray-975 hover:bg-gray-975">
        <RiDashboard3Line />
        {warnings.length > 0 && <div className="absolute right-1.5 top-2 h-[9px] w-[9px] rounded-full border border-white bg-[#ff5655]" />}
      </MenuButton>
      <MenuItems
      ...
      >
        <MenuItem>
          <div className="flex items-center justify-between p-6">
            <h3 className="m-0 text-lg font-bold text-black">État du service</h3>
            <Link to="/admin-warning" className="flex items-center text-blue-france">
              <span>Détails</span>
              <RiArrowDropRightLine className="mt-1 text-lg" />
            </Link>
          </div>
        ...
        {state && (
          <MenuItem>
            <Link to="/admin-warning" className="flex items-center justify-between gap-6 p-6 hover:bg-gray-950 border-t border-gray-900">
              <div className="flex w-6 items-center">
                <LogoSvg alt="API Engagement" />
              </div>
              <div className="flex flex-1 flex-col gap-2">
        ...
        {warnings.length ? (
          <>
              ...
              return (
                <MenuItem key={w._id}>
                  <Link
                    ...
                    className="flex items-center justify-between gap-6 p-6 hover:bg-gray-950 border-t border-gray-borde"
                  >
                    <div className="flex w-6 items-center">
                      <span>{label.emoji}</span>
                    </div>
                    <div className="flex flex-1 flex-col gap-2">
                      <p className="m-0 text-xs text-gray-425">{w.publisherName}</p>
                      <div>
                        <span className="truncate rounded bgbg-[#FEECC2] p-1 text-center text-xs font-semibold uppercase textbg-[#716043]">{label.name}</span>
                      </div>
                      <h4 className="m-0 text-sm font-bold text-black">{w.title}</h4>
            ...
            {warnings.length > 3 && (
              <MenuItem>
                <Link to={`/admin-warning`} className={`flex items-center justify-end gap-6 p-6 hover:bg-gray-950 border-t border-gray-900`}>
                  <div className="flex text-blue-france">
                    <span>Voir toutes les alertes</span>
                    <RiArrowDropRightLine className="mt-1 text-lg" />
                  </div>
...
const AccountMenu = () => {
  ...
  return (
    <Menu>
      <MenuButton className="button flex cursor-pointer items-center gap-4 data-[focus]:bg-gray-975 hover:bg-gray-975">
        <div className="bg-blue-france w-8 h-8 flex justify-center items-center rounded-full">
          <RiUserLine className="text-white" />
        </div>
        <div className="space-y-0 text-left">
          <p className="text-blue-france">{user.firstname}</p>
          <p className="text-sm text-gray-425">{user.publishers.length ? (user.role === "admin" ? "Administrateur" : "Utilisateur") : publisher.name}</p>
        </div>
      ...
      >
        <MenuItem>
          <Link to="my-account" className="block w-full p-4 text-sm data-[focus]:bg-gray-975">
            Mon compte
          </Link>
        </MenuItem>
        <MenuItem>
          <button className="w-full p-4 text-sm text-left data-[focus]:bg-gray-975" onClick={handleLogout}>
            Se déconnecter
          </button>
        </MenuItem>
      </MenuItems>
