            ...
            options
              ...
              .map((option, i) => (
                <Fragment key={i}>
                  ...
                  <li
                    className={`relative flex gap-4 cursor-default select-none list-none items-center justify-between px-6 py-3 text-sm hover:bg-gray-100 hover:text-blue-dark bg-white text-black`}
                    tabIndex={-1}
                    onClick={() => {
                      onChange(selected.includes(option.value) ? { value: selected.filter((v) => v !== option.value) } : { value: [...selected, option.value] });
                    }}
                  >
                    {selected.includes(option.value) && <RiCheckLine className="absolute left-1 top-1/2 -translate-y-1/2 text-sm text-blue-dark" />}
