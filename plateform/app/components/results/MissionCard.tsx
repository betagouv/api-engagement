...
export default function MissionCard({ item, index, userScoringId, onDebugClick }: Props) {
  const detailPath = userScoringId ? `/results/${userScoringId}/missions/${item.mission.id}` : `/missions/${item.mission.id}`;
  ...
  return (
    <Link to={detailPath} className="mission-card-link block">
      <article className="mission-card flex min-h-[430px] flex-col overflow-hidden">
        ...
        <div className="mission-card__body flex flex-1 flex-col px-5 py-6 sm:px-7 sm:py-7">
          {item.mission.domain && <span className="mission-card__badge mb-5 w-fit rounded-full px-4 py-2 text-sm font-medium leading-none">{item.mission.domain}</span>}
