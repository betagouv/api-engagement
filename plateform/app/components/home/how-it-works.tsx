  ...
  return (
    <section className="fr-container relative z-10 flex flex-col gap-4 py-8 md:py-12 lg:items-center lg:gap-8 lg:py-16">
      <div className="md:mx-auto! md:text-center">
        <h2 className="fr-h1 mb-0! md:mx-auto md:mb-3! md:max-w-3xl">
          Toute l'offre publique d'<Highlight className="bg-green-tilleul-verveine-925">engagement</Highlight> réunie en un seul endroit
        </h2>
      ...
      <ul role="list" className="list-none! mt-6! mb-0! flex w-full flex-col gap-4 p-0! md:grid md:grid-cols-2 md:gap-4 lg:grid-cols-4">
        {FEATURES.map((feature) => (
          <li
            key={feature.icon}
            className="list-none! marker:content-none! flex items-center gap-2 md:bg-background md:h-full md:flex-col md:gap-4 md:p-10 md:text-center md:shadow-tile lg:w-full lg:max-w-68 lg:p-6!"
          >
            <span className="bg-blue-ecume-975 dark:bg-white p-2 rounded-full flex size-12 shrink-0 items-center justify-center md:size-20 md:rounded-none dark:md:p-1 dark:md:rounded-full md:bg-transparent">
      ...
      <div className="flex flex-col gap-3 md:items-center md:mt-4">
        <button type="button" onClick={onStartQuiz} className="fr-btn fr-btn--secondary fr-btn--lg w-full! justify-center md:w-auto!">
          Trouver ma mission
        </button>
        <p className="fr-text--sm text-mention-grey fr-mb-0! text-center! md:mt-8!">Près de 400 000 occasions de s’engager partout en France</p>
      </div>
    </section>
  );
}
