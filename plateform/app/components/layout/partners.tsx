...
type PartnerId = "jva" | "service_civique" | "spv" | "gendarmerie";
...
// `campaignUrls` : liens de redirection propres à la page appelante (campagnes dédiées à une landing,
// pour attribuer les clics à cette page). Les partenaires absents gardent leur lien par défaut.
export default function Partners({ style = "default", campaignUrls = {} }: { style?: "default" | "compact"; campaignUrls?: Partial<Record<PartnerId, string>> }) {
  return (
      ...
      <div className={`fr-container ${style === "compact" ? "py-6! md:py-12! px-6!" : "fr-py-8w"}`}>
        ...
        >
          {PARTNERS.map((partner) => (
            <li key={partner.id} className={`flex items-start gap-2 ${style === "compact" ? "flex-1" : "gap-4"}`}>
              ...
              <div className="flex-1">
                <p className="fr-mb-0 font-bold">
                  <a
                    href={campaignUrls[partner.id] ?? partner.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    title={`${partner.name} - nouvelle fenêtre`}
                    className="text-title-grey bg-none!"
                  >
                    {partner.name}
                  </a>
                </p>
